# 🏗️ Notification Service - Architecture

## System Architecture

```
┌─────────────────────────────────────────────────────────────────┐
│                        API Gateway / Client                      │
│                     (NestJS, Express, etc.)                      │
└───────────────────────────┬─────────────────────────────────────┘
                            │
                            │ gRPC Request (HTTP/2)
                            │ + Metadata (auth, request-id)
                            ▼
┌─────────────────────────────────────────────────────────────────┐
│                    Notification Service                          │
│                         (Port 50051)                             │
├─────────────────────────────────────────────────────────────────┤
│                                                                   │
│  ┌───────────────────────────────────────────────────────┐      │
│  │           GrpcAuthInterceptor (Middleware)             │      │
│  │  • Check authorization token                           │      │
│  │  • Validate x-service-id                               │      │
│  │  • Log x-request-id                                    │      │
│  └───────────────────────────────────────────────────────┘      │
│                           │                                       │
│                           ▼                                       │
│  ┌───────────────────────────────────────────────────────┐      │
│  │         NotificationController (@Controller)           │      │
│  │                                                         │      │
│  │  @GrpcMethod('SendNotification')                       │      │
│  │  @GrpcMethod('SendBatchNotifications')                 │      │
│  │  @GrpcMethod('GetNotificationStatus')                  │      │
│  │  @GrpcMethod('HealthCheck')                            │      │
│  └───────────────────────────────────────────────────────┘      │
│                           │                                       │
│                           ▼                                       │
│  ┌───────────────────────────────────────────────────────┐      │
│  │           NotificationService (Business Logic)         │      │
│  │                                                         │      │
│  │  • processNotification()                               │      │
│  │  • sendNotification()                                  │      │
│  │  • sendBatchNotifications()                            │      │
│  │  • getNotificationStatus()                             │      │
│  │  • In-memory storage (Map)                             │      │
│  └───────────────────────────────────────────────────────┘      │
│                           │                                       │
│                           ▼                                       │
│  ┌───────────────────────────────────────────────────────┐      │
│  │            GrpcExceptionFilter (Error Handler)         │      │
│  │  • HTTP → gRPC status mapping                          │      │
│  │  • Error logging                                       │      │
│  │  • Structured error response                           │      │
│  └───────────────────────────────────────────────────────┘      │
│                                                                   │
└─────────────────────────────────────────────────────────────────┘
```

## Request Processing Flow

```
Client Request
     │
     ├─► 1. gRPC Transport Layer (HTTP/2)
     │      • TLS/mTLS handshake
     │      • Protocol Buffers deserialization
     │
     ├─► 2. GrpcAuthInterceptor
     │      • Extract metadata
     │      • Validate auth token
     │      • Log request
     │
     ├─► 3. Controller (@GrpcMethod)
     │      • Route to correct method
     │      • DTO validation (class-validator)
     │
     ├─► 4. Service (Business Logic)
     │      • Process notification
     │      • Generate UUID
     │      • Store in memory/DB
     │      • Send via provider (EMAIL/SMS/PUSH)
     │
     ├─► 5. Response
     │      • Protocol Buffers serialization
     │      • Return to client
     │
     └─► 6. Error Handling (if error)
            • GrpcExceptionFilter
            • Map to gRPC status code
            • Structured error response
```

## Notification Types

```
┌──────────────────────────────────────────────────────┐
│              Notification Types                       │
├──────────────────────────────────────────────────────┤
│                                                       │
│  📧 EMAIL                                             │
│     • Welcome emails                                 │
│     • Password reset                                 │
│     • Newsletters                                    │
│     • Transactional emails                           │
│     └─► Providers: SendGrid, AWS SES, SMTP          │
│                                                       │
│  📱 SMS                                               │
│     • OTP codes                                      │
│     • Alerts                                         │
│     • Verification codes                             │
│     └─► Providers: Twilio, AWS SNS                  │
│                                                       │
│  🔔 PUSH                                              │
│     • Mobile notifications                           │
│     • Desktop notifications                          │
│     • Real-time alerts                               │
│     └─► Providers: FCM, APNs                        │
│                                                       │
│  🔗 WEBHOOK                                           │
│     • Third-party integrations                       │
│     • Event callbacks                                │
│     • API notifications                              │
│     └─► HTTP POST to external URLs                  │
│                                                       │
└──────────────────────────────────────────────────────┘
```

## Security Layers

```
┌─────────────────────────────────────────────────────────┐
│                    Security Layers                       │
└─────────────────────────────────────────────────────────┘
         │
         ├─► Layer 1: Transport Security (mTLS)
         │   ┌──────────────────────────────────────┐
         │   │  • TLS 1.3 encryption                │
         │   │  • Client certificate verification   │
         │   │  • Server certificate verification   │
         │   │  • CA-signed certificates            │
         │   └──────────────────────────────────────┘
         │
         ├─► Layer 2: Authentication (Metadata)
         │   ┌──────────────────────────────────────┐
         │   │  authorization: Bearer JWT           │
         │   │  x-service-id: caller-service        │
         │   │  x-request-id: trace-id              │
         │   └──────────────────────────────────────┘
         │
         ├─► Layer 3: Input Validation
         │   ┌──────────────────────────────────────┐
         │   │  • class-validator decorators        │
         │   │  • Type checking (TypeScript)        │
         │   │  • Enum validation                   │
         │   │  • Range validation (Min/Max)        │
         │   └──────────────────────────────────────┘
         │
         └─► Layer 4: Rate Limiting (planned)
             ┌──────────────────────────────────────┐
             │  • Per service limits                │
             │  • Per user limits                   │
             │  • Priority-based throttling         │
             └──────────────────────────────────────┘
```

## Priority Queue

```
┌─────────────────────────────────────────────────┐
│           Priority Queue (planned)               │
├─────────────────────────────────────────────────┤
│                                                  │
│  🔴 URGENT (Priority: 3)                         │
│     ├─► Immediate processing                    │
│     ├─► Timeout: 2s                             │
│     └─► Retry: 5 times                          │
│                                                  │
│  🟠 HIGH (Priority: 2)                           │
│     ├─► Fast processing                         │
│     ├─► Timeout: 5s                             │
│     └─► Retry: 3 times                          │
│                                                  │
│  🟡 NORMAL (Priority: 1)                         │
│     ├─► Standard processing                     │
│     ├─► Timeout: 10s                            │
│     └─► Retry: 3 times                          │
│                                                  │
│  🟢 LOW (Priority: 0)                            │
│     ├─► Background processing                   │
│     ├─► Timeout: 30s                            │
│     └─► Retry: 1 time                           │
│                                                  │
└─────────────────────────────────────────────────┘
```

## Data Flow - SendNotification

```
┌──────────────┐
│   Client     │
└──────┬───────┘
       │
       │ SendNotificationRequest {
       │   event_id: "evt_123"
       │   user_id: "user_456"
       │   type: "EMAIL"
       │   template_id: "welcome"
       │   payload: {...}
       │   priority: "HIGH"
       │ }
       ▼
┌──────────────────────────────────────┐
│    Notification Service              │
│                                      │
│  1. Generate UUID                    │
│     notification_id = uuid()         │
│                                      │
│  2. Validate Input                   │
│     class-validator checks           │
│                                      │
│  3. Process Notification             │
│     ├─► Select provider (email)     │
│     ├─► Render template             │
│     ├─► Send via provider           │
│     └─► Update status               │
│                                      │
│  4. Store State                      │
│     Map.set(notification_id, {      │
│       ...request,                    │
│       status: "SENT",                │
│       created_at: timestamp          │
│     })                               │
│                                      │
│  5. Return Response                  │
│     SendNotificationResponse {       │
│       success: true                  │
│       notification_id: "notif_abc"  │
│       status: "SENT"                 │
│       timestamp: 1703001234567       │
│     }                                │
└──────────────────────────────────────┘
       │
       ▼
┌──────────────┐
│   Client     │
│  (Response)  │
└──────────────┘
```

## Retry Logic

```
┌─────────────────────────────────────────────────┐
│              Retry Mechanism                     │
├─────────────────────────────────────────────────┤
│                                                  │
│  Attempt 1 ──► Send ──► Success? ──► Return     │
│       │                    │                     │
│       │                    │ Failure             │
│       │                    ▼                     │
│       │              Wait (backoff)              │
│       │                    │                     │
│       ▼                    │                     │
│  Attempt 2 ──► Send ──► Success? ──► Return     │
│       │                    │                     │
│       │                    │ Failure             │
│       │                    ▼                     │
│       │              Wait (backoff)              │
│       │                    │                     │
│       ▼                    │                     │
│  Attempt 3 ──► Send ──► Success? ──► Return     │
│                            │                     │
│                            │ Failure             │
│                            ▼                     │
│                    Mark as FAILED                │
│                    Store error details           │
│                    Return error response         │
│                                                  │
│  Exponential Backoff:                            │
│  • Attempt 1: immediate                          │
│  • Attempt 2: wait 1s                            │
│  • Attempt 3: wait 2s                            │
│  • Attempt 4: wait 4s                            │
│  • Attempt 5: wait 8s                            │
│                                                  │
└─────────────────────────────────────────────────┘
```

## Deployment Architecture

```
┌────────────────────────────────────────────────────────────┐
│                    Kubernetes Cluster                       │
├────────────────────────────────────────────────────────────┤
│                                                             │
│  ┌──────────────────────────────────────────────────┐     │
│  │              Load Balancer (Service)              │     │
│  │                ClusterIP: 50051                   │     │
│  └───────────────────────┬──────────────────────────┘     │
│                          │                                  │
│              ┌───────────┼───────────┐                     │
│              │           │           │                      │
│              ▼           ▼           ▼                      │
│  ┌─────────────┐ ┌─────────────┐ ┌─────────────┐         │
│  │   Pod 1     │ │   Pod 2     │ │   Pod 3     │         │
│  │ (Replica 1) │ │ (Replica 2) │ │ (Replica 3) │         │
│  │             │ │             │ │             │         │
│  │  Container: │ │  Container: │ │  Container: │         │
│  │  notification│ │  notification│ │  notification│        │
│  │  -service   │ │  -service   │ │  -service   │         │
│  │             │ │             │ │             │         │
│  │  Port: 50051│ │  Port: 50051│ │  Port: 50051│         │
│  │             │ │             │ │             │         │
│  │  Resources: │ │  Resources: │ │  Resources: │         │
│  │  CPU: 250m  │ │  CPU: 250m  │ │  CPU: 250m  │         │
│  │  Mem: 256Mi │ │  Mem: 256Mi │ │  Mem: 256Mi │         │
│  └─────────────┘ └─────────────┘ └─────────────┘         │
│                                                             │
│  ┌──────────────────────────────────────────────────┐     │
│  │      HorizontalPodAutoscaler (HPA)               │     │
│  │  • Min replicas: 3                               │     │
│  │  • Max replicas: 10                              │     │
│  │  • Target CPU: 70%                               │     │
│  │  • Target Memory: 80%                            │     │
│  └──────────────────────────────────────────────────┘     │
│                                                             │
└────────────────────────────────────────────────────────────┘
```

## Microservices Communication

```
┌─────────────────┐
│   API Gateway   │
│   (Port 3000)   │
└────────┬────────┘
         │ HTTP/REST
         │ (from users)
         ▼
┌─────────────────────────────────────┐
│        API Gateway Service          │
│    (NestJS with gRPC Client)        │
└────────┬────────────────────────────┘
         │
         │ gRPC (HTTP/2)
         │ Port 50051
         │
         ├──────────────────────────────────┐
         │                                  │
         ▼                                  ▼
┌───────────────────┐          ┌───────────────────────┐
│   Notification    │          │   Other Services      │
│     Service       │          │   (User, Order, etc.) │
│   (Port 50051)    │          │                       │
└─────────┬─────────┘          └───────────────────────┘
          │
          │ Future integrations
          │
          ├─► Email Provider (SendGrid, SES)
          │
          ├─► SMS Provider (Twilio, SNS)
          │
          ├─► Push Provider (FCM, APNs)
          │
          └─► Message Queue (RabbitMQ, Redis)
```

## Notification Status Flow

```
PENDING ──► SENT ──► DELIVERED
   │                    │
   │                    │
   ├──► RETRY ─────┬────┘
   │               │
   │               │
   └──► FAILED ◄───┘

PENDING:   Created, waiting to send
SENT:      Sent to provider
DELIVERED: Delivery confirmed
RETRY:     Retrying
FAILED:    Error, not delivered
```

## Observability Stack (Planned)

```
┌─────────────────────────────────────────────────────┐
│                  Observability Stack                 │
├─────────────────────────────────────────────────────┤
│                                                      │
│  Logging (Winston/Pino)                             │
│  ┌────────────────────────────────────────┐         │
│  │  • Structured JSON logs                │         │
│  │  • Log levels: debug/info/warn/error   │         │
│  │  • Request/Response logging            │         │
│  │  • Error stack traces                  │         │
│  └────────────────────────────────────────┘         │
│                                                      │
│  Metrics (Prometheus)                               │
│  ┌────────────────────────────────────────┐         │
│  │  • Total requests                      │         │
│  │  • Success/failure rate                │         │
│  │  • Response time (histogram)           │         │
│  │  • Active connections (gauge)          │         │
│  │  • Queue depth                          │         │
│  └────────────────────────────────────────┘         │
│                                                      │
│  Tracing (OpenTelemetry + Jaeger)                  │
│  ┌────────────────────────────────────────┐         │
│  │  • Distributed tracing                 │         │
│  │  • Span tracking                       │         │
│  │  • Service dependency map              │         │
│  │  • Latency analysis                    │         │
│  └────────────────────────────────────────┘         │
│                                                      │
│  Dashboards (Grafana)                               │
│  ┌────────────────────────────────────────┐         │
│  │  • Real-time metrics visualization     │         │
│  │  • Alerts and notifications            │         │
│  │  • SLA monitoring                      │         │
│  └────────────────────────────────────────┘         │
│                                                      │
└─────────────────────────────────────────────────────┘
```

This architecture provides:
- ✅ Scalability (Kubernetes + HPA)
- ✅ Security (mTLS + Auth)
- ✅ Reliability (Retry + Error Handling)
- ✅ Performance (gRPC + HTTP/2)
- ✅ Observability (Logging + Metrics + Tracing)

